        ...
        while True:
            try:
                ...
                try:
                    ...
                    logger.warning(f"Failed to decode frame from client: {decode_err}")
                    continue
                ...
                logger.error(f"Error processing client frame: {inner_e}")
                continue
            
    except WebSocketDisconnect:
        logger.info(f"Client disconnected: {camera_id}")
    except Exception as e:
        logger.error(f"WebSocket fatal error for {camera_id}: {e}")
    finally:
        try:
            await websocket.close()
        except:
